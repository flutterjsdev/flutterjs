import { Widget, StatefulWidget } from '../../core/widget.js';
...
class AssetImage extends ImageProvider {
  constructor(assetPath, { package = null, scale = 1.0 } = {}) {
    super(IMAGE_SOURCE_TYPES.asset);
    this.assetPath = assetPath;
    this.package = package;
    this.scale = scale;
    this.key = `asset:${package}:${assetPath}:${scale}`;
  }
  ...
  _resolveAssetPath() {
    const basePath = this.package ? `/packages/${this.package}` : '/assets';
...
class Image extends StatefulWidget {
  ...
  static asset(assetPath, {
    ...
    const provider = new AssetImage(assetPath, { package, scale: scale || 1.0 });
    return new Image({ key, image: provider, ...otherOptions });
  }
