...
class WidgetPropertyValidationPass {
  ...
  // =========================================================================
  // WIDGET PROPERTY VALIDATORS
  // =========================================================================

  /// Validate MaterialApp configuration
  void _validateMaterialApp(FlutterWidgetConstructorIR widget) {
    final properties = _extractPropertyMap(widget);

    // Check for required properties
    if (!properties.containsKey('home') && !properties.containsKey('routes')) {
      _addWidgetIssue(
        severity: IssueSeverity.error,
        message: 'MaterialApp must have either home or routes property',
        widget: widget,
        code: 'MATERIAL_APP_MISSING_HOME',
      );
    }

    // Check theme configuration
    if (properties.containsKey('theme')) {
      _validateThemeData(properties['theme']!, widget);
    }
  }

  /// Validate Scaffold configuration
  void _validateScaffold(FlutterWidgetConstructorIR widget) {
    final properties = _extractPropertyMap(widget);

    // Scaffold should have body or floatingActionButton at minimum
    if (!properties.containsKey('body') &&
        !properties.containsKey('floatingActionButton') &&
    ...
    // Check AppBar configuration if present
    if (properties.containsKey('appBar')) {
      _validateAppBar(properties['appBar']!, widget);
    }
  }
  ...
  void _validateAppBar(ExpressionIR appBarExpr, FlutterWidgetConstructorIR parent) {
    if (appBarExpr is FlutterWidgetConstructorIR) {
      ...
      if (!properties.containsKey('title')) {
        _addWidgetIssue(
          severity: IssueSeverity.info,
          message: 'AppBar should have a title property',
          widget: appBarExpr,
          code: 'APPBAR_MISSING_TITLE',
        );
      }
    }
  }
  ...
  void _validateThemeData(ExpressionIR themeExpr, FlutterWidgetConstructorIR parent) {
    if (themeExpr is FlutterWidgetConstructorIR &&
      ...
      // Check for colorScheme or primarySwatch (one is required for modern Flutter)
      if (!properties.containsKey('colorScheme') &&
          !properties.containsKey('primarySwatch') &&
          !properties.containsKey('primaryColor')) {
        _addWidgetIssue(
          severity: IssueSeverity.warning,
          message: 'ThemeData should define colorScheme or primarySwatch',
          widget: themeExpr,
          code: 'THEME_DATA_MISSING_COLOR_SCHEME',
        );
      }
    }
  }

  /// Validate FloatingActionButton
  void _validateFloatingActionButton(FlutterWidgetConstructorIR widget) {
    final properties = _extractPropertyMap(widget);

    // onPressed is required
    if (!properties.containsKey('onPressed')) {
      _addWidgetIssue(
        severity: IssueSeverity.error,
        message: 'FloatingActionButton requires onPressed callback',
        widget: widget,
        code: 'FAB_MISSING_ON_PRESSED',
      );
    }

    // Should have child or label
    if (!properties.containsKey('child') && !properties.containsKey('label')) {
      _addWidgetIssue(
        severity: IssueSeverity.warning,
        message: 'FloatingActionButton should have child or label',
        widget: widget,
        code: 'FAB_MISSING_CHILD',
      );
    }
  }

  /// Validate Center/Align widgets
  void _validateCenterWidget(FlutterWidgetConstructorIR widget) {
    final properties = _extractPropertyMap(widget);

    if (!properties.containsKey('child')) {
      _addWidgetIssue(
        severity: IssueSeverity.error,
        message: '${widget.className} requires a child widget',
        widget: widget,
        code: 'SINGLE_CHILD_WIDGET_MISSING_CHILD',
      );
    }
  }

  /// Validate Column/Row
  void _validateMultiChildWidget(FlutterWidgetConstructorIR widget) {
    final properties = _extractPropertyMap(widget);

    if (!properties.containsKey('children')) {
      _addWidgetIssue(
        severity: IssueSeverity.error,
        message: '${widget.className} requires children property',
        widget: widget,
        code: 'MULTI_CHILD_WIDGET_MISSING_CHILDREN',
      );
    } else {
      // Validate children is a list
      final childrenExpr = properties['children'];
      if (childrenExpr is ListExpressionIR) {
        if (childrenExpr.elements.isEmpty) {
          _addWidgetIssue(
            severity: IssueSeverity.info,
            message: '${widget.className} has empty children list',
            widget: widget,
            code: 'EMPTY_CHILDREN_LIST',
          );
        }
      }
    }
  ...
  void _validateTextWidget(FlutterWidgetConstructorIR widget) {
    final properties = _extractPropertyMap(widget);
    ...
    if (widget.positionalArguments.isEmpty &&
      ...
      _addWidgetIssue(
        ...
        widget: widget,
        code: 'TEXT_MISSING_DATA',
      );
    }
  }

  /// Validate ListView/GridView
  void _validateScrollableWidget(FlutterWidgetConstructorIR widget) {
    final properties = _extractPropertyMap(widget);

    if (widget.className.contains('ListView') ||
        widget.className.contains('GridView')) {
      // Check for builder pattern or children
      final isBuilder = widget.className.endsWith('.builder');

      if (!isBuilder && !properties.containsKey('children')) {
        _addWidgetIssue(
          severity: IssueSeverity.error,
          message:
              '${widget.className} requires children or use .builder() constructor',
          widget: widget,
          code: 'SCROLLABLE_MISSING_CHILDREN',
        );
      }

      if (isBuilder && !properties.containsKey('itemBuilder')) {
        _addWidgetIssue(
          severity: IssueSeverity.error,
          message: '${widget.className} requires itemBuilder callback',
          widget: widget,
          code: 'BUILDER_MISSING_ITEM_BUILDER',
        );
      }
    }
  }
  ...
  /// Extract properties as a Map for easy lookup
  Map<String, ExpressionIR> _extractPropertyMap(
    FlutterWidgetConstructorIR widget,
  ) {
    final map = <String, ExpressionIR>{};
    for (final namedArg in widget.namedArgumentsDetailed) {
      map[namedArg.name] = namedArg.value;
    }
    return map;
  }

  /// Generic issue adder
  void _addWidgetIssue({
    required IssueSeverity severity,
    required String message,
    required FlutterWidgetConstructorIR widget,
    required String code,
  }) {
    widgetIssues.add(
      AnalysisIssue(
        id: 'widget_issue_${widgetIssues.length}',
        code: code,
        severity: severity,
        category: IssueCategory.flutterWidget,
        message: message,
        sourceLocation: widget.sourceLocation,
        suggestion: _generateSuggestion(code, widget),
      ),
    );
  }

  String _generateSuggestion(String code, FlutterWidgetConstructorIR widget) {
    switch (code) {
      case 'MATERIAL_APP_MISSING_HOME':
        return 'Set the home property to the initial route widget: MaterialApp(home: MyHomePage())';
      case 'EMPTY_SCAFFOLD':
        return 'Add a body property or floatingActionButton to your Scaffold';
      case 'FAB_MISSING_ON_PRESSED':
        return 'Add onPressed callback: FloatingActionButton(onPressed: () { /* action */ })';
      case 'TEXT_MISSING_DATA':
        return 'Provide text data: Text("Hello") or Text(data: variable)';
      case 'SCROLLABLE_MISSING_CHILDREN':
        return 'Use ${widget.className}.builder() for better performance with large lists';
      case 'BUILDER_MISSING_ITEM_BUILDER':
        return 'Add itemBuilder callback: ListView.builder(itemBuilder: (context, index) => ...)';
      case 'EMPTY_CHILDREN_LIST':
        return 'Add widgets to the children list or remove the empty container';
      default:
        return 'Check widget documentation for required properties';
    }
  }
}
