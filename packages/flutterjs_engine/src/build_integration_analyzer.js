...
class BuildAnalyzer {
  constructor(buildIntegration) {
    ...
    console.log(
      chalk.magenta(`  Has config.packages: ${!!this.config?.packages}`)
    );
    if (this.config?.packages) {
      console.log(
        chalk.magenta(
          `  Config packages: ${Object.keys(this.config.packages).join(", ")}`
        )
      );
    ...
    this.importRewriter = new ImportRewriter({
      ...
      projectRoot: buildIntegration.projectRoot, // ✅ NEW: Pass project root for package_map.json
    });
  ...
  async phase1_analyze() {
    ...
    try {
      ...
      // ✅ NEW: Detect if source is already JS (generated by Dart)
      ...
      if (isJS) {
        if (this.config.debugMode) {
          console.log(
            chalk.blue(
              "  ℹ️  Detected JavaScript source - Using Regex Analysis"
            )
          );
        }
      ...
      this.integration.analysis = {
        ...
        imports: finalImports, // ✅ Updated with vdom
        metadata: {
      ...
      if (this.config.debugMode) {
        console.log(
          chalk.gray(`  Widgets: ${this.integration.analysis.widgets.count}`)
        );
        ...
        console.log(
          chalk.gray(
            `  Imports: ${
              Object.keys(this.integration.analysis.imports).length
            }`
          )
        ); // Adjusted log
        ...
        console.log(
          chalk.gray(
            `  Root: ${this.integration.analysis.metadata.rootWidget}\n`
          )
        );
      }
    } catch (error) {
    ...
    for (const pkg of corePackages) {
      if (!importObject[pkg]) {
        ...
        if (this.config.debugMode) {
          console.log(
            chalk.yellow(
              `  ℹ️  Auto-added ${pkg} to imports (required core dependency)`
            )
          );
        }
      }
  ...
  async phase2_resolveDependencies() {
    ...
    try {
        ...
        for (const [name, pkg] of Object.entries(this.config.packages)) {
          if (!this.integration.resolution.packages.has(name)) {
            if (this.config.debugMode) {
              console.log(
                chalk.gray(
                  `  Simulating resolution for config package: ${name}`
                )
              );
            }
      ...
      spinner.succeed(
        chalk.green(
          `✔ Resolved ${this.integration.resolution.packages.size} packages`
        )
      );
    } catch (error) {
  ...
  async phase4_collectPackages() {
    ...
    try {
      if (
        ...
        this.integration.resolution.packages.size === 0
      ) {
      ...
      this.integration.collection = {
        ...
        failedFiles:
          session.globalErrors.length > 0 ? session.globalErrors : [],
      ...
      if (this.config.debugMode) {
        ...
        console.log(
          chalk.gray(`  Packages: ${stats.successful}/${stats.total}`)
        );
  ...
  async phase5_transformCode() {
    ...
    try {
      ...
      const importAnalysisResult =
        await this.importRewriter.analyzeImportsWithResolution(
          sourceCode,
      ...
      if (this.config.debugMode) {
        console.log(
          chalk.gray(
            `  Framework imports found: ${importAnalysisResult.stats.framework}`
          )
        );
        console.log(
          chalk.gray(
            `  External imports: ${importAnalysisResult.stats.external}`
          )
        );
        console.log(
          chalk.gray(`  Local imports: ${importAnalysisResult.stats.local}`)
        );
        console.log(
          chalk.gray(
            `  Packages loaded: ${importAnalysisResult.packageExports.size}`
          )
        );

        const importCount = importMapObj.imports
          ? Object.keys(importMapObj.imports).length
          : 0;
        console.log(chalk.gray(`  Import map entries: ${importCount}`));

        if (
          importMapObj.imports &&
          Object.keys(importMapObj.imports).length > 0
        ) {
          ...
          for (const [key, value] of Object.entries(importMapObj.imports)) {
            if (count >= 5) {
              console.log(
                chalk.gray(
                  `    ... and ${
                    Object.keys(importMapObj.imports).length - 5
                  } more`
                )
              );
              break;
        ...
        console.log(
          chalk.gray(
            `  Transformations: ${this.integration.transformed.transformations}\n`
          )
        );
      }
    } catch (error) {
  ...
  async phase6_initializeRuntime() {
    ...
    try {
      ...
      if (this.config.debugMode) {
        ...
        console.log(
          chalk.gray("  (Actual runtime will initialize in browser)\n")
        );
      }
  ...
  async phase7_buildWidgets() {
    ...
    try {
      ...
      if (this.config.debugMode) {
        ...
        console.log(
          chalk.gray(`  Found ${Object.keys(widgetMetadata).length} widgets\n`)
        );
      }
