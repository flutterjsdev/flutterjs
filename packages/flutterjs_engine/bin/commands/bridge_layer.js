/**
 * ============================================================================
 * WORKING AppBridge - Simplified for Counter App
 * ============================================================================
 * 
 * This connects:
 * 1. Analyzer output (metadata)
 * 2. User code (main.js - widget classes)
 * 3. VNode system (build, render)
 * 4. DOM (browser rendering)
 * 
 * Location: dist/app_bridge.js
 */

/**
 * Simplified BuildContext - Provides theme and services to widgets
 */
class BuildContext {
  constructor(element, bridge, services = {}) {
    this.element = element;
    this.bridge = bridge;
    this.theme = services.theme || {};
    this.mediaQuery = services.mediaQuery || {};
...
/**
 * Simplified StateManager - Tracks state changes
 */
class StateManager {
  constructor(bridge) {
    this.bridge = bridge;
  ...
  register(state, options = {}) {
    const stateId = options.id || `state_${this.states.size}`;
...
/**
 * Simplified VNodeBuilder - Converts widgets to VNodes
 */
class VNodeBuilder {
  static build(widget, context) {
    if (!widget) return null;

    // Already a VNode (plain object with tag)
    if (widget && widget.tag) {
      return widget;
    }

    // String or number - return as is
    ...
    if (Array.isArray(widget)) {
      return widget.map(w => this.build(w, context));
    }

    // Widget object - call build() method
    if (widget && typeof widget.build === 'function') {
      try {
        const result = widget.build(context);
        return this.build(result, context);
      } catch (error) {
        console.error(`Error building ${widget.constructor.name}:`, error);
        return null;
      }
    }
...
/**
 * Simplified VNodeRenderer - Renders VNodes to DOM
 */
class VNodeRenderer {
  static render(vnode, rootElement, options = {}) {
    if (!rootElement) {
